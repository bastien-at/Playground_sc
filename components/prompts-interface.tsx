...
import { FileText, Loader2, RefreshCw, ChevronRight, Terminal } from "lucide-react";
...
export function PromptsInterface() {
  ...
  const fetchFiles = React.useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Erreur",
        description: "Impossible de charger la liste des prompts",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const loadFileContent = React.useCallback(async (file: MarkdownFile) => {
    ...
    try {
      const res = await fetch(`/api/prompts?file=${encodeURIComponent(file.path)}`);
      if (!res.ok) throw new Error("Erreur");
      const data = await res.json();
        ...
        <Card className="flex-1 overflow-y-auto">
          <div className="p-2 space-y-1">
            {files.map((file) => (
              ...
              >
                <Terminal className={cn("h-4 w-4", selectedFile?.path === file.path ? "text-white/80" : "text-muted-foreground")} />
            ...
            {files.length === 0 && (
              <p className="text-xs text-center py-4 text-muted-foreground italic">
                Aucun prompt trouvé dans prompts-data/
              </p>
            )}
          </div>
        </Card>
      ...
      <Card className="flex-1 overflow-hidden flex flex-col">
        <div className="border-b px-6 py-4 flex items-center justify-between bg-muted/30">
          <div className="flex items-center gap-3">
            <Terminal className="h-5 w-5 text-[#005162]" />
            <div>
              <h3 className="font-semibold text-foreground">
                {selectedFile?.name || "Sélectionnez un prompt"}
              </h3>
            </div>
          </div>
        </div>
        ...
        <CardContent className="flex-1 overflow-y-auto p-8">
          {contentLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-muted-foreground">Sélectionnez un prompt pour voir son contenu</p>
            </div>
          )}
        </CardContent>
      </Card>
